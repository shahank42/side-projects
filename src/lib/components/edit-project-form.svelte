...
<script lang="ts">
	...
	import type { ActionData, PageData } from '../../routes/edit-project/[projectId]/$types';

	export let data: PageData;
	export let form: ActionData;

	// There's work cut out for me here
	// a LOOOOOT of things that could be done better
	// heck the naming is even inconsistent :-P
	...
	$: console.log(data.dbProjects);

	onMount(() => {
		$formData.title = data.dbProjects?.title as string;
		$formData.status = data.dbProjects?.status as string;
		$formData.description = data.dbProjects?.description as string;
		$formData.githubRepoLink = data.dbProjects?.githubRepoLink as string;
	});

	$: if (form?.updatedData) {
		$formData.title = form.updatedData.title as string;
		$formData.status = form.updatedData.status as string;
		$formData.description = form.updatedData.description as string;
		$formData.githubRepoLink = form.updatedData.githubRepoLink as string;
	}
</script>
